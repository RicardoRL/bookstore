...
class PedidoController extends Controller
{
  ...
  public function showOneOrder(Cliente $cliente, Pedido $pedido)
  {
    ...
    foreach($pedido->libros()->withTrashed()->get() as $libro)
    {
      $libros["modelo"][] = Libro::withTrashed()->where('id', $libro->id)->first();
      $libros["cantidad"][] = $libro->pivot->cantidad;
    }

    $size = count($libros['modelo']);

    return view('layouts_cliente.clientePedidoIndividual', compact('pedido', 'domicilio', 'libros', 'size'));
  }
}
